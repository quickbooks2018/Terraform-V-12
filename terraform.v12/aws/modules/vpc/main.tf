...
resource "aws_subnet" "public-subnets" {
  count                   = var.enabled && length(var.azs) > 0 ? length(var.azs) : length(data.aws_availability_zones.azs.names)
...
resource "aws_subnet" "database" {
  count = var.enabled && length(var.azs) > 0 ? length(var.azs) : length(data.aws_availability_zones.azs.names)
  cidr_block = "${element(var.vpc-database_subnets-cidr,count.index)}"
  vpc_id = aws_vpc.vpc[0].id
  availability_zone = data.aws_availability_zones.azs.names[count.index]
