...
module "vpc" {
  ...
  name                                = "vpc-2-us-east-1"
  stage                               = "dev"
  map_public_ip_on_launch             = "false"
  total-nat-gateway-required          = "1"
  create_database_subnet_group        = "false"
  vpc-cidr                            = "10.1.0.0/16"
  vpc-public-subnet-cidr              = ["10.1.1.0/24","10.1.2.0/24"]
  vpc-private-subnet-cidr             = ["10.1.4.0/24","10.1.5.0/24"]
  vpc-database_subnets-cidr           = ["10.1.7.0/24", "10.1.8.0/24"]
}
...
module "sg-tgw" {
  ...
  ingress-rule-1-cidrs = ["10.0.0.0/16"]
  ingress-rule-1-description = "Allow Traffic From VPC1"

  # Rule-4
  ingress-rule-4-from-port   = 0
  ingress-rule-4-to-port     = 0
  ingress-rule-4-protocol    = -1
  ingress-rule-4-cidrs       = ["10.128.0.0/16"]
  ingress-rule-4-description = "Allow Traffic From VPC1-eu-west-1"
  ...
  ingress-rule-3-cidrs       = ["10.1.0.0/16"]
...
module "ec2-tgw" {
  ...
  namespace                     = "cloudgeeksca-tgw"
  stage                         = "dev"
  name                          = "ec2-vpc-2-us-east-1"
  key_name                      = "transit-gateway-vpc-2"
