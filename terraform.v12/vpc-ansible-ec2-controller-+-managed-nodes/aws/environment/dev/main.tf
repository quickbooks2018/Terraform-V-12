...
module "sg2" {
  ...
  security_group_name     = "ansible-Ref"
  vpc_id                  = module.vpc.vpc-id
}


module "ansible-eip" {
  source = "../../modules/eip/ansible"
  name                         = "ansible"
  instance                     = module.ec2-ansible-master.id[0]
}

module "ec2-keypair" {
  source = "../../modules/aws-ec2-keypair"
  key-name      = "ansible"
  public-key    = file("../../modules/secrets/ansible.pub")
}

module "ec2-ansible-master" {
  source                        = "../../modules/aws-ec2"
  namespace                     = "cloudgeeks.ca"
  stage                         = "dev"
  name                          = "ansible"
  key_name                      = "ansible"
  user_data                     = file("../../modules/aws-ec2/user-data/ansible/ansible-master.sh")
  instance_count                = 1
...
module "ec2-ansible-slave" {
  source                        = "../../modules/aws-ec2"
  namespace                     = "cloudgeeks.ca"
  stage                         = "dev"
  name                          = "slave"
  key_name                      = "ansible"
  user_data                     = file("../../modules/aws-ec2/user-data/ansible/ansible-slave.sh")
  instance_count                = 1
  ami                           = "ami-0fc61db8544a617ed"
  instance_type                 = "t3a.medium"
  associate_public_ip_address   = "true"
